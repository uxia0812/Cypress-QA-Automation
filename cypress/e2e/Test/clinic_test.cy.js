describe('EasyMedi 병원관리자 - 에이전트 관리 테스트', () => {
  it('제휴 신청 테스트', () => {
  ...
  it('제휴 병원 관리 테스트', () => {
    ...
    .eq(0).click(); // 첫 번째 제휴 병원 관리 버튼 클릭
    
    cy.contains('상세 정보').click(); // 상세 정보 버튼 클릭
    cy.get('.space-y-3 > :nth-child(2) > .space-x-2 > .flex').clear().type('10') // 수수료 요율 변경
    cy.contains('button', '저장').click(); // 저장 버튼 클릭
    cy.get('.min-h-\[60px\]').type('테스트 메모') // 메모 입력
    cy.get('textarea[placeholder="에이전트에 대한 메모를 작성하세요..."]').type('테스트 메모');
